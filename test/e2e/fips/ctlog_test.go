...
var _ = Describe("Securesign FIPS - ctlog signer test", Ordered, func() {
	...
	Describe("Reject non-FIPS ctlog TLS then accept FIPS-compliant TLS", func() {
		...
		It("Update ctlog TLS to use FIPS-compliant secret and verify readiness", func(ctx SpecContext) {
			Eventually(func(g Gomega) error {
				g.Expect(cli.Get(ctx, types.NamespacedName{Name: s.Name, Namespace: namespace.Name}, s)).To(Succeed())

				s.Spec.Ctlog.TLS = v1alpha1.TLS{
					CertRef: &v1alpha1.SecretKeySelector{
						LocalObjectReference: v1alpha1.LocalObjectReference{
							Name: "my-ctlog-tls-secret",
						},
						Key: "tls.crt",
					},
					PrivateKeyRef: &v1alpha1.SecretKeySelector{
