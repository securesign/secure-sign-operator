...
var _ = Describe("TSA update", Ordered, func() {
	...
	Describe("modify tsa signer and certificate chain", func() {
		...
		It("verify TSA and TUF", func(ctx SpecContext) {
		...
		It("verify new configuration", func(ctx SpecContext) {
			...
			Expect(cli.Get(ctx, types.NamespacedName{Namespace: namespace.Name, Name: t.Status.Signer.FileSigner.PrivateKeyRef.Name}, privateKeySecret)).To(Succeed())
	...
	Describe("Update NTP config", func() {
		...
		It("verify TSA", func(ctx SpecContext) {
			tsa.Verify(ctx, cli, namespace.Name, s.Name)
		})
		...
		It("verify new configuration", func(ctx SpecContext) {
			...
			Expect(pod.Spec.Volumes).To(ContainElements(And(
				...
				WithTransform(func(v v1.Volume) string { return v.ConfigMap.Name }, Equal(t.Status.NTPMonitoring.NtpConfigRef.Name)))))

			cm := &v1.ConfigMap{}
			Expect(cli.Get(ctx, types.NamespacedName{Namespace: namespace.Name, Name: t.Status.NTPMonitoring.NtpConfigRef.Name}, cm)).To(Succeed())
