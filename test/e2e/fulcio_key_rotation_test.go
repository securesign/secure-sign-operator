...
var _ = Describe("Fulcio cert rotation test", Ordered, func() {
	...
	Describe("Fulcio cert rotation", func() {
		...
		It("Update TUF repository", func() {
			...
			Expect(cli.List(ctx, tufPodList, runtimeCli.InNamespace(namespace.Name), runtimeCli.MatchingLabels{kubernetes.ComponentLabel: tufAction.ComponentName})).To(Succeed())
			Expect(tufPodList.Items).To(HaveLen(1))

			Expect(kubernetes2.CopyFromPod(ctx, config.GetConfigOrDie(), tufPodList.Items[0], "/var/www/html", filepath.Join(tufRepoWorkdir, "tuf-repo"))).To(Succeed())
