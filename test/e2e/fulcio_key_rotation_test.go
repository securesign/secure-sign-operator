...
var _ = Describe("Fulcio cert rotation test", Ordered, func() {
	...
	var (
		...
		testRegistry    *support.TestRegistry
	...
	BeforeAll(func(ctx SpecContext) {
		testRegistry = support.DeployTestRegistry(ctx, cli, namespace.Name)
		DeferCleanup(testRegistry.Close)
		targetImageName = testRegistry.PrepareImage(ctx)
	})
	...
	Describe("Fulcio cert rotation", func() {
		...
		It("Use cosign cli", func(ctx SpecContext) {
			tas.VerifyByCosign(ctx, cli, s, targetImageName)
			newImage := testRegistry.PrepareImage(ctx)
