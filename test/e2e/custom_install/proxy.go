...
var _ = Describe("Securesign install in proxy-env", Ordered, func() {
	...
	Describe("Successful installation with fake-proxy env", func() {
		BeforeAll(func() {
			...
			createProxyServer(cli, ctx, hostname, namespace.Name)

			installOperatorWithProxyConf(cli, ctx, hostname, namespace.Name)
...
func createProxyServer(cli runtimeCli.Client, ctx context.Context, hostname string, ns string) {
...
func installOperatorPodWithProxyConf(proxyHostname string, ns string) *v1.Pod {
	image := os.Getenv("TEST_MANAGER_IMAGE")
	Expect(image).ToNot(BeNil(), "TEST_MANAGER_IMAGE can't be nil")
...
func installOperatorWithProxyConf(cli runtimeCli.Client, ctx context.Context, proxyHostname string, ns string) {
