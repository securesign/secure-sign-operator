...
> **Important:** The certificate chain's public key **must** match the KMS signing key. Fulcio verifies this at startup and will fail with a fatal error if the keys differ. For Vault/OpenBao, create the transit key with `exportable=true` to extract the private key for certificate generation.
