...
// +kubebuilder:validation:XValidation:rule="(has(self.rekor.attestations.enabled) && !self.rekor.attestations.enabled) || !self.rekor.attestations.url.startsWith('file://') || !has(self.rekor.replicas) || !(self.rekor.replicas > 1) || (has(self.rekor.pvc.accessModes) && 'ReadWriteMany' in self.rekor.pvc.accessModes)",message="When rich attestation storage is enabled, and it's URL starts with 'file://', then rekor pvc.accessModes must contain 'ReadWriteMany' for replicas greater than 1."
