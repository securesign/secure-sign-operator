...
func GenerateRSAPKCS8PEM(t *testing.T, keyBits int) []byte {
	t.Helper()
	der, _ := x509.MarshalPKCS8PrivateKey(mustGenerateRSAKey(&testing.T{}, keyBits))
...
func GenerateRSAPKCS1PEM(t *testing.T, keyBits int) []byte {
	der := x509.MarshalPKCS1PrivateKey(mustGenerateRSAKey(&testing.T{}, keyBits))
	return pem.EncodeToMemory(&pem.Block{
...
func GenerateRSAPKIXPublicKeyPEM(t *testing.T, keyBits int) []byte {
	t.Helper()
	der, _ := x509.MarshalPKIXPublicKey(&mustGenerateRSAKey(&testing.T{}, keyBits).PublicKey)
	return pem.EncodeToMemory(&pem.Block{
		Type:  "PUBLIC KEY",
		Bytes: der,
	})
...
func GenerateRSAPKCS1PublicKeyPEM(t *testing.T, keyBits int) []byte {
	t.Helper()
	der := x509.MarshalPKCS1PublicKey(&mustGenerateRSAKey(&testing.T{}, keyBits).PublicKey)
	return pem.EncodeToMemory(&pem.Block{
		Type:  "RSA PUBLIC KEY",
		Bytes: der,
	})
