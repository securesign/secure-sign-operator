...
func testWrapper() func(*rhtasv1alpha1.Trillian) *wrapper[*rhtasv1alpha1.Trillian] {
	return Wrapper(
		func(t *rhtasv1alpha1.Trillian) rhtasv1alpha1.TLS { return t.Spec.LogServer.TLS },
		func(t *rhtasv1alpha1.Trillian) rhtasv1alpha1.TLS { return t.Status.LogServer.TLS },
		func(t *rhtasv1alpha1.Trillian, tls rhtasv1alpha1.TLS) { t.Status.LogServer.TLS = tls },
		nil,
	)
}
...
func TestCanHandle(t *testing.T) {
	type env struct {
		...
		isOpenShift bool
		isEnabled   func(*rhtasv1alpha1.Trillian) bool
	}
	tests := []struct {
		name      string
		env       env
		...
		},
		{
			...
			env: env{
				...
				isEnabled: func(_ *rhtasv1alpha1.Trillian) bool { return false },
			},
			canHandle: false,
		},
		{
...
func TestHandle(t *testing.T) {
	tests := []struct {
		...
		expectedStatusTLS rhtasv1alpha1.TLS
	}{
		{
			name: "user cert specified — copies spec to status",
			specTLS: rhtasv1alpha1.TLS{
				CertRef: &rhtasv1alpha1.SecretKeySelector{
					LocalObjectReference: rhtasv1alpha1.LocalObjectReference{Name: "user-cert"},
					Key:                  "tls.crt",
				},
				PrivateKeyRef: &rhtasv1alpha1.SecretKeySelector{
					LocalObjectReference: rhtasv1alpha1.LocalObjectReference{Name: "user-key"},
					Key:                  "tls.key",
				},
			},
		...
		},
		{
			...
			expectedStatusTLS: rhtasv1alpha1.TLS{
				CertRef: &rhtasv1alpha1.SecretKeySelector{
					LocalObjectReference: rhtasv1alpha1.LocalObjectReference{Name: "test-instance-test-tls"},
					Key:                  "tls.crt",
				},
				PrivateKeyRef: &rhtasv1alpha1.SecretKeySelector{
					LocalObjectReference: rhtasv1alpha1.LocalObjectReference{Name: "test-instance-test-tls"},
		...
		},
		{
			...
			specTLS: rhtasv1alpha1.TLS{
				CertRef: &rhtasv1alpha1.SecretKeySelector{
					LocalObjectReference: rhtasv1alpha1.LocalObjectReference{Name: "cert"},
					Key:                  "tls.crt",
				},
			},
			expectedStatusTLS: rhtasv1alpha1.TLS{
