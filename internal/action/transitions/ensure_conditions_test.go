...
func TestEnsureConditions_CanHandle(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			conditions: []metav1.Condition{
				{Type: "A", Status: metav1.ConditionTrue, Reason: "Ready"},
				{Type: "B", Status: metav1.ConditionTrue, Reason: "Ready"},
			},
	...
	tests := []struct {
		...
		},
		{
			...
			verify: func(g gomega.Gomega, obj *rhtasv1.CTlog) {
				...
				g.Expect(a.Reason).To(gomega.Equal("Ready"))
		...
		},
		{
			...
			conditions: []metav1.Condition{
				{Type: "base", Status: metav1.ConditionTrue, Reason: "Ready"},
			},
