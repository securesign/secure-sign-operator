...
func TestServerConfig_Handle(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "Find existing Ctlog configuration",
			env: env{
				spec: rhtasv1alpha1.CTlogSpec{
					ServerConfigRef: nil,
					Trillian:        rhtasv1alpha1.TrillianService{Port: ptr.To(int32(80))},
				...
				objects: []client.Object{
					...
					&v1.Secret{
						ObjectMeta: metav1.ObjectMeta{
							...
							Namespace:   "default",
							Labels:      labels,
							Annotations: map[string]string{configGenerationAnnotation: "1"},
						},
		...
		{
			name: "Do not take old config and remove it",
			env: env{
				spec: rhtasv1alpha1.CTlogSpec{
					ServerConfigRef: nil,
					Trillian:        rhtasv1alpha1.TrillianService{Port: ptr.To(int32(80))},
				...
				objects: []client.Object{
					...
					&v1.Secret{
						ObjectMeta: metav1.ObjectMeta{
							...
							Namespace:   "default",
							Labels:      labels,
							Annotations: map[string]string{configGenerationAnnotation: "0"},
						},
						Data: map[string][]byte{},
					},
				},
			...
			want: want{
				...
				verify: func(g Gomega, instance *rhtasv1alpha1.CTlog, cli client.WithWatch) {
					...
					g.Expect(k8sErrors.IsNotAcceptable(err)).To(BeTrue())
				},
			},
		},
	}
