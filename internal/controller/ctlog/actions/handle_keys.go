...
func (g handleKeys) setupKeys(ns string, instanceSpec *v1alpha1.CTlogStatus) (*utils.KeyConfig, error) {
	var (
		err    error
		config = &utils.KeyConfig{}
	)
	if instanceSpec.PrivateKeyPasswordRef != nil {
		config.PrivateKeyPass, err = k8sutils.GetSecretData(g.Client, ns, instanceSpec.PrivateKeyPasswordRef)
		if err != nil {
			return nil, err
		}
	}

	if instanceSpec.PrivateKeyRef == nil {
		return utils.CreatePrivateKey(config.PrivateKeyPass)
	} else {

		config.PrivateKey, err = k8sutils.GetSecretData(g.Client, ns, instanceSpec.PrivateKeyRef)
		if err != nil {
			return nil, err
		}
	}

	if instanceSpec.PublicKeyRef == nil {
		return utils.GeneratePublicKey(config)
	} else {
		config.PublicKey, err = k8sutils.GetSecretData(g.Client, ns, instanceSpec.PrivateKeyRef)
		if err != nil {
			return nil, err
		}
		return config, nil
...
func (g handleKeys) discoverPrivateKey(ctx context.Context, instance *v1alpha1.CTlog, newKeyStatus *v1alpha1.CTlogStatus) {
	...
	for _, partialPrivateSecret := range partialPrivateSecrets.Items {
		...
		scr, err := k8sutils.GetSecret(g.Client, partialPrivateSecret.Namespace, partialPrivateSecret.Name)
		if err != nil {
			g.Logger.Error(err, "problem with invalidating private key secret", "namespace", instance.Namespace)
		}
		delete(scr.Labels, CTLogPrivateLabel)
		err = g.Client.Update(ctx, scr)
		if err != nil {
			g.Logger.Error(err, "problem with invalidating private key secret", "namespace", instance.Namespace)
		}
...
func (g handleKeys) discoverPubliceKey(ctx context.Context, instance *v1alpha1.CTlog, newKeyStatus *v1alpha1.CTlogStatus) {
	...
	for _, partialPubSecret := range partialPubSecrets.Items {
		...
		scr, err := k8sutils.GetSecret(g.Client, partialPubSecret.Namespace, partialPubSecret.Name)
		if err != nil {
			g.Logger.Error(err, "problem with invalidating private key secret", "namespace", instance.Namespace)
		}
		delete(scr.Labels, CTLPubLabel)
		err = g.Client.Update(ctx, scr)
		if err != nil {
