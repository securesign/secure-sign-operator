...
func TestCanHandle_PKCS11_CondTrue_SameHash(t *testing.T) {
	...
	// Pre-compute hash and set PKCS11Condition with matching hash.
	hash := pkcs11SpecHash(instance.Spec.Signer.PKCS11)
	meta.SetStatusCondition(&instance.Status.Conditions, metav1.Condition{
		Type:    PKCS11Condition,
		Status:  metav1.ConditionTrue,
		Reason:  ReasonResolved,
		Message: fmt.Sprintf("%s%s]", pkcs11HashPrefix, hash),
	})
...
func TestCanHandle_PKCS11_CondTrue_DiffHash(t *testing.T) {
	...
	meta.SetStatusCondition(&instance.Status.Conditions, metav1.Condition{
		...
		Status:  metav1.ConditionTrue,
		Reason:  ReasonResolved,
		Message: fmt.Sprintf("%sdeadbeef]", pkcs11HashPrefix),
	})
...
func TestHandle_Rotation_FieldsUnchanged(t *testing.T) {
	...
	// Set PKCS11Condition True with matching hash -- CanHandle should return false.
	hash := pkcs11SpecHash(instance.Spec.Signer.PKCS11)
	meta.SetStatusCondition(&instance.Status.Conditions, metav1.Condition{
		Type:               PKCS11Condition,
		Status:             metav1.ConditionTrue,
...
func TestHandle_Rotation_PinSecretRefChanged(t *testing.T) {
	...
	// Set PKCS11Condition True with hash from old spec.
	oldHash := pkcs11SpecHash(instance.Spec.Signer.PKCS11)
	meta.SetStatusCondition(&instance.Status.Conditions, metav1.Condition{
		Type:    PKCS11Condition,
		Status:  metav1.ConditionTrue,
		Reason:  ReasonResolved,
		Message: fmt.Sprintf("%s%s]", pkcs11HashPrefix, oldHash),
	})
