...
func TestIngress_CanHandle(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:           "ingress is enabled and console is not ready",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			g := NewWithT(t)
			instance := rhtasv1alpha1.Console{
				Spec: rhtasv1alpha1.ConsoleSpec{
					UI: rhtasv1alpha1.ConsoleUI{
						ExternalAccess: rhtasv1alpha1.ExternalAccess{
							Enabled: tt.externalAccess,
						},
					},
				},
				Status: rhtasv1alpha1.ConsoleStatus{
