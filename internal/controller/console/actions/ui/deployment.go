...
func (i deployAction) ensureUIDeployment(instance *rhtasv1.Console, sa string, labels map[string]string) func(*v1.Deployment) error {
	return func(dp *v1.Deployment) error {
		...
		apiProtocol := "http"
		curlOptions := "-sf"
		if consoleUtils.UseTLSApi(instance) {
			apiProtocol = "https"
			// Use -k for init container only since it doesn't have CA certs mounted
			curlOptions = "-sfk"
		}
		apiURL := fmt.Sprintf("%s://console-api.%s.svc:8080", apiProtocol, instance.Namespace)
		...
		initContainer.Command = []string{
			"/bin/sh",
			"-c",
			fmt.Sprintf(`max_retries=50
count=0
until curl %s %s/healthz; do
