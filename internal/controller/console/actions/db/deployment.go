...
func (i deployAction) CanHandle(ctx context.Context, instance *rhtasv1alpha1.Console) bool {
...
func (i deployAction) Handle(ctx context.Context, instance *rhtasv1alpha1.Console) *action.Result {
	...
	if result, err = kubernetes.CreateOrUpdate(ctx, i.Client,
		...
		ensure.Optional(consoleUtils.UseTLSDb(instance), i.ensureTLS(statusTLS(instance))),
	); err != nil {
		return i.Error(ctx, fmt.Errorf("could not create Console DB: %w", err), instance, metav1.Condition{
...
func (i deployAction) ensureDbDeployment(instance *rhtasv1alpha1.Console, sa string, labels map[string]string) func(deployment *v2.Deployment) error {
	return func(dp *v2.Deployment) error {
		...
		container.Image = images.Registry.Get(images.ConsoleDb)
		...
		// Env variables from secret console-db
