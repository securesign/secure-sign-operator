...
import (
	...
	"github.com/securesign/operator/internal/controller/console/actions/api"
	"github.com/securesign/operator/internal/controller/console/actions/db"
...
func (r *consoleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	var instance rhtasv1alpha1.Console
	...
	actions := []action.Action[*rhtasv1alpha1.Console]{
		transitions.NewToPendingPhaseAction[*rhtasv1alpha1.Console](func(t *rhtasv1alpha1.Console) []string {
			return []string{actions.UICondition, actions.ApiCondition, actions.DbCondition}
		}),
		...
		transitions.NewToCreatePhaseAction[*rhtasv1alpha1.Console](),
		...
		api.NewDeployAction(),
		api.NewCreateServiceAction(),
		api.NewCreateMonitorAction(),

		ui.NewDeployAction(), // issue here (blocks next)
		ui.NewCreateServiceAction(),
		ui.NewIngressAction(),
		// ui.NewCreateMonitorAction(),

		transitions.NewToInitializePhaseAction[*rhtasv1alpha1.Console](),

		db.NewInitializeAction(),
		api.NewInitializeAction(),
		// ui.NewInitializeAction(), // issue here (blocks next)

		transitions.NewToReadyPhaseAction[*rhtasv1alpha1.Console](),
	}
