...
func (i deployAction) ensurePKCS11Deployment(instance *rhtasv1.Fulcio, sa string, labels map[string]string, dp *v1.Deployment) error {
	...
	if pkcs11Config.ConfigRef == (rhtasv1.SecretKeySelector{}) {
		return errors.New("pkcs11 configRef is not specified")
	}
	...
	args := []string{
		...
		fmt.Sprintf("--log_type=%s", utils.GetOrDefault(instance.GetAnnotations(), annotations.LogType, string(constants.Prod))),
		"--ca=pkcs11ca",
		fmt.Sprintf("--pkcs11-config-path=%s/%s", PKCS11ConfigMountPath, pkcs11Config.ConfigRef.Key),
	...
	reconcileInitContainers(&template.Spec, instance.Spec.InitContainers)
	...
	pkcs11ConfigVol.VolumeSource = core.VolumeSource{
		Secret: &core.SecretVolumeSource{
			SecretName: pkcs11Config.ConfigRef.Name,
		},
	}
...
func reconcileInitContainers(podSpec *core.PodSpec, specs []rhtasv1.PKCS11InitContainerSpec) {
