...
func TestCanHandle_PKCS11_CondTrue_SameHash(t *testing.T) {
	...
	instance := pkcs11Instance()
	// Set annotation + condition with matching hash.
	currentHash := computePKCS11Hash(instance)
	instance.SetAnnotations(map[string]string{annotations.PKCS11SpecHash: currentHash})
...
func TestCanHandle_PKCS11_CondTrue_DiffHash(t *testing.T) {
	...
	// Set annotation with stale hash + condition.
	instance.SetAnnotations(map[string]string{annotations.PKCS11SpecHash: "stale-old-hash"})
	meta.SetStatusCondition(&instance.Status.Conditions, metav1.Condition{
		Type:               PKCS11Condition,
		Status:             metav1.ConditionTrue,
		Reason:             state.Ready.String(),
