...
var _ = Describe("Rekor hot update test", func() {
	Context("Rekor update test", func() {
		...
		It("should successfully reconcile a custom resource for Rekor", func() {
			...
			found := &rhtasv1.Rekor{}
			Eventually(func(g Gomega) *common.SecretKeySelector {
			...
			pubKeyData, err := kubernetes.GetSecretData(suite.Client(), Namespace, &common.SecretKeySelector{
			...
			Eventually(func(g Gomega) error {
				found := &rhtasv1.Rekor{}
				g.Expect(suite.Client().Get(ctx, typeNamespaceName, found)).Should(Succeed())
				found.Spec.Signer.KeyRef = &common.SecretKeySelector{
					LocalObjectReference: common.LocalObjectReference{
						Name: "key-secret",
					},
					Key: "private",
