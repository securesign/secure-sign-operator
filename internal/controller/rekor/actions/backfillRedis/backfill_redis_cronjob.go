...
func ensureRedisParams() func(*redis.RedisOptions, *v1.Container) {
	return func(options *redis.RedisOptions, container *v1.Container) {
		...
		container.Args[0] += fmt.Sprintf(" --redis-hostname=\"%s\"", options.Host)

		if options.Port != "" {
			container.Args[0] += fmt.Sprintf(" --redis-port=\"%s\"", options.Port)
		}

		if options.Password != "" {
			container.Args[0] += fmt.Sprintf(" --redis-password=\"%s\"", options.Password)
		}
...
func ensureMysqlParams() func(string, *v1.Container) {
	return func(url string, container *v1.Container) {
		container.Args[0] += fmt.Sprintf(" --mysql-dsn=\"%s\"", url)
	}
}
