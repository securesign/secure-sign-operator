...
const PvcNameFormat = "rekor-monitor-%s-pvc"
...
func (i createPvcAction) CanHandle(_ context.Context, instance *rhtasv1alpha1.Rekor) bool {
	...
	return c.Reason == constants.Creating && instance.Status.MonitorPvcName == ""
}
...
func (i createPvcAction) Handle(ctx context.Context, instance *rhtasv1alpha1.Rekor) *action.Result {
	...
	if instance.Spec.Pvc.Name != "" {
		instance.Status.MonitorPvcName = instance.Spec.Pvc.Name
	...
	if result, err = kubernetes.CreateOrUpdate(ctx, i.Client, pvc,
		...
		return i.Error(ctx, fmt.Errorf("could not create monitor PVC: %w", err), instance)
	}
	...
	instance.Status.MonitorPvcName = pvc.Name
	return i.StatusUpdate(ctx, instance)
}
