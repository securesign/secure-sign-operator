	...
	return func(job *batchv1.Job) error {
		...
		for _, key := range instance.Spec.Keys {
			...
			case "rekor.pub":
				...
				env.Value = apis.ServiceAsUrl(&instance.Spec.Rekor)
			case "ctfe.pub":
				...
				env.Value = fmt.Sprintf("%s/%s", apis.ServiceAsUrl(&instance.Spec.Ctlog), instance.Spec.Ctlog.Prefix)
			case "fulcio_v1.crt.pem":
				env = kubernetes.FindEnvByNameOrCreate(container, "FULCIO_URL")
				env.Value = apis.ServiceAsUrl(&instance.Spec.Fulcio)
			case "tsa.certchain.pem":
				env = kubernetes.FindEnvByNameOrCreate(container, "TSA_URL")
				env.Value = fmt.Sprintf("%s%s", apis.ServiceAsUrl(&instance.Spec.Tsa), tsaActions.TimestampPath)
			}
		}
