...
if [ -z "${FULCIO_URL:-}" ]; then echo "Error: FULCIO_URL is not set."; exit 1; fi
if [ -z "${REKOR_URL:-}" ]; then echo "Error: REKOR_URL is not set."; exit 1; fi
...
if [ -z "${OIDC_ISSUERS:-}" ]; then echo "Error: OIDC_ISSUERS is not set."; exit 1; fi
...
add_signing_config() {
    ...
    if [ -n "${TSA_URL:-}" ]; then
        if [[ "$TSA_URL" != *"/api/v1/timestamp" ]]; then
...
if [ "$NEEDS_UPDATE" = true ]; then
    ...
    # Store backup even if no update was needed
    mkdir -p "$TUF_REPO/backup"
    mv "$WORKDIR/$BACKUP_NAME" "$TUF_REPO/backup/$BACKUP_NAME"
fi
