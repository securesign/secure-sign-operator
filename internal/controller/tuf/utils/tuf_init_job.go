	...
	return func(job *batchv1.Job) error {
		...
		for _, key := range instance.Spec.Keys {
			...
			case "rekor.pub":
				...
				args = append(args, "--rekor-uri", apis.ServiceAsUrl(&instance.Spec.Rekor))
			case "ctfe.pub":
				...
				args = append(args, "--ctlog-uri", fmt.Sprintf("%s/%s", apis.ServiceAsUrl(&instance.Spec.Ctlog), instance.Spec.Ctlog.Prefix))
			case "fulcio_v1.crt.pem":
				args = append(args, "--fulcio-cert", filepath.Join(secretsMonthPath, key.Name))
				args = append(args, "--fulcio-uri", apis.ServiceAsUrl(&instance.Spec.Fulcio))
			case "tsa.certchain.pem":
				args = append(args, "--tsa-cert", filepath.Join(secretsMonthPath, key.Name))
				args = append(args, "--tsa-uri", fmt.Sprintf("%s%s", apis.ServiceAsUrl(&instance.Spec.Tsa), tsaActions.TimestampPath))
			}
		}
