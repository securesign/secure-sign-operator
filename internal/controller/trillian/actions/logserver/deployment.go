...
func (i deployAction) Handle(ctx context.Context, instance *rhtasv1alpha1.Trillian) *action.Result {
	...
	if err != nil {
		...
		return i.ErrorWithStatusUpdate(ctx, fmt.Errorf("could not create Trillian server: %w", err), instance)
	}

	if err = controllerutil.SetControllerReference(instance, server, i.Client.Scheme()); err != nil {
		return i.Error(fmt.Errorf("could not set controller reference for server: %w", err))
	}
