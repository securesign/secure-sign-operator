...
func NewCreateIngressAction() action.Action[rhtasv1alpha1.Trillian] {
	return &createIngressAction{}
}

type createIngressAction struct {
	action.BaseAction
}

func (i createIngressAction) Name() string {
	return "create ingress"
}

func (i createIngressAction) CanHandle(ctx context.Context, instance *rhtasv1alpha1.Trillian) bool {
	c := meta.FindStatusCondition(instance.Status.Conditions, constants.Ready)
	return (c.Reason == constants.Creating || c.Reason == constants.Ready) && instance.Spec.ExternalAccess.Enabled
}

func (i createIngressAction) Handle(ctx context.Context, instance *rhtasv1alpha1.Trillian) *action.Result {
	var updated bool
	ok := types.NamespacedName{Name: actions.LogserverDeploymentName, Namespace: instance.Namespace}
	...
	ingress, err := kubernetes.CreateIngress(ctx, i.Client, *svc, instance.Spec.ExternalAccess, string(actions.ServerPortName), labels)
	if err != nil {
