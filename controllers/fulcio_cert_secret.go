...
func (r *SecuresignReconciler) ensureFulcioSecret(ctx context.Context, m *rhtasv1alpha1.Securesign, namespace string, secretName string) (*corev1.Secret,
	error) {
	...
	secret := &corev1.Secret{
		...
		Data: map[string][]byte{
			"public":   []byte(m.Spec.FulcioPublicKey),
			"private":  []byte(m.Spec.FulcioPrivateKey),
			"cert":     []byte(m.Spec.FulcioCert),
			"password": []byte(m.Spec.FulcioPassword),
		},
	}
